...
export default function ConsultationPage() {
  ...
  const fetchSlots = async () => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/director_slots?is_available=eq.true&order=slot_time`,
      {
        headers: {
  ...
  // toKST()로 통일 — 해당 날짜 슬롯 목록 (날짜만 비교라 toKST 사용)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      alert((err.error || '오류가 발생했습니다.') + (err.detail ? '\n\n상세: ' + err.detail : ''));
    }
    setLoading(false);
  };
        ...
        {step === 'form' && (
          <div className="max-w-md mx-auto bg-white rounded-2xl shadow-sm border p-8">
            ...
            <div className="bg-blue-50 rounded-xl p-4 mb-6">
              <p className="text-blue-700 font-bold">예약 시간</p>
              <p className="text-blue-600">
                {selectedDate} {formatTime(selectedSlot.slot_time)}
              </p>
            </div>
