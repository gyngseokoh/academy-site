...
export default async function ColumnsPage() {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/columns?is_published=eq.true&select=*&order=sort_order.asc`,
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="bg-gradient-to-br from-blue-900 to-blue-700 text-white py-16 px-6 text-center">
        <p className="text-blue-300 text-sm font-bold mb-2 tracking-widest">COLUMNS</p>
            ...
            {list.map((c: any) => (
              <div key={c.id} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition">
                <div className="flex items-center gap-2 mb-2">
                  {c.category && (
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${CAT_COLORS[c.category] ?? 'bg-gray-100 text-gray-600'}`}>
                      {c.category}
                    </span>
                  )}
                ...
                {c.content && (
                  <p className="text-gray-600 text-sm leading-relaxed whitespace-pre-line border-t pt-3 mt-3">
                    {c.content}
                  </p>
                )}
              </div>
