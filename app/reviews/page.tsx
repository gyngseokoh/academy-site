...
export default async function ReviewsPage() {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/reviews?is_published=eq.true&select=*&order=created_at.desc`,
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="bg-gradient-to-br from-blue-900 to-blue-700 text-white py-16 px-6 text-center">
        <p className="text-blue-300 text-xs font-bold mb-2 tracking-widest uppercase">Reviews</p>
            ...
            {list.map((r: any) => (
              <div key={r.id} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {r.category && (
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${CATEGORY_COLORS[r.category] ?? 'bg-gray-100 text-gray-600'}`}>{r.category}</span>
                  )}
                </div>
